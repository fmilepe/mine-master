...
posicao(X,Y) :- acabou(FIM), FIM, writeln('Jogo encerrado, de make para começar um novo jogo.').
...
posicao2(X,Y) :- valor(X,Y,N), N = 0, write_jogada(X,Y),nb_getval(stream,STR),write(STR,'valor('), write(STR,X),write(STR,','),write(STR,Y),write(STR,',0).'),nl(STR),assert(v((X,Y),N)), X1 is X -1, X2 is X + 1, Y1 is Y-1, Y2 is Y + 1, posicao2(X1,Y1),posicao2(X,Y1),posicao2(X2,Y1), posicao2(X1,Y), posicao2(X2,Y),posicao2(X1,Y2),posicao2(X,Y2),posicao2(X2,Y2).
